...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Causs\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=554792091566:+55 6285638976\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('connecting', () => {
		start('2', 'Pera ae..Carregando...')
	})
	client.on('open', () => {
		success('2', 'Bot Halerquin ON')
	})
...
sᴇᴊᴀ ʙᴇᴍ ᴠɪɴᴅᴏ(ᴀ) ɪɴᴛᴇʀᴀᴊᴀ ᴘᴀʀᴀ ɴᴀᴏ sᴇʀ ʀᴇᴍᴏᴠɪᴅᴏ...❤️
				...
				teks = `Tchau @${num.split('@')[0]}Vai embora mesmo kkk😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '[❗] Este comando só pode ser usado por admins, seu Membro comum 😅! ❌',
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau otário, Eu Avisei 😔🤙")
		}, 0)
	}
		...
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('Manoo, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau otário, Só obedeço as ordens dos Adm 😔🤙")
		}, 0)
	}
		...
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('Opoha🤬, nao fale essas coisas, é errado, 😤mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} voce vai tomar ban deste grupo em 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau otário, Se as Regras vc não leu o Problema é seu! 😔🤙")
		}, 0)
	}
		...
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('Idiotaa, nao fale essas coisas, Não é certo mano, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK, As regras são claras, quem não obedece topa tapa na cara 😔🤙")
		}, 0)
	}
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau manin, Já tava na Hora 😔🤙")
		}, 0)
	}
	
	if (messagesC.includes("fdp")){
			client.updatePresence(from, Presence.composing)
			reply("Vc que é, Respeita Moço 😑")
	}
		...
		if (messagesC.includes("vsfd")){
			client.updatePresence(from, Presence.composing)
			reply("bora juntos? Idiota😒")
	}
	
		if (messagesC.includes("cadebot")){
			client.updatePresence(from, Presence.composing)
			reply("olha eu aqui, Tá com saudades já 🥰?")
	}
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				...
				case 'ssweb':
					...
					break
                case 'ytkomen':
					if (args.length < 1) return reply('Cadê o texto manin?')
					gh = body.slice(9)
					usnm = gh.split("|")[0];
					cmn = gh.split("|")[1];
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modoanime':
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Hmm, ok!')
					if ((args[0]) === 'on') {
				...
				case 'loli2':
					...
					client.sendMessage(from, nyeee, image, { caption: 'Oni chaan!!', quoted: mek })
					break
					case 'xd':				
